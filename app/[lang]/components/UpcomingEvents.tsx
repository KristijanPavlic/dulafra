...
const UpcomingEvents: React.FC<UpcomingEventsProps> = ({ titleEvents, noEvents, btnDelete, btnDeletion }) => {
  const { user } = useUser();
  ...
  const filteredImages = images.filter(image => image.folder === "upcoming_events");
  ...
  return (
    <div
      ...
      id="upcomingEvents"
    >
      <h1 className="text-center text-3xl font-bold uppercase text-[#001120] mb-5">
        {titleEvents}
      </h1>
      {isLoading ? (
        ...
        <div className="mt-10 grid sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10 gap-y-16 text-center">
